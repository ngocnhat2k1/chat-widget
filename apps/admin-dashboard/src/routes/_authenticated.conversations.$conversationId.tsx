...
const VISITOR_FIELD_LABELS: Record<string, string> = {
  currentPageUrl: "Trang hiện tại",
...
function VisitorInfoPanel({
  ...
  const entries = metadata ? Object.entries(metadata).filter(([, v]) => v) : [];

  return (
    <aside className="hidden lg:flex lg:flex-col w-72 border-l border-gray-200 bg-white overflow-y-auto">
      ...
      <div className="px-5 py-4 space-y-4 text-sm">
        <div>
          ...
          <p className="text-gray-700 break-all">{visitorId}</p>
        </div>
        {visitorEmail && (
          <div>
            <p className="text-xs text-gray-400">Email</p>
            <p className="text-gray-700 break-all">{visitorEmail}</p>
          </div>
        )}
        {entries.length > 0 ? (
          entries.map(([key, value]) => (
            <div key={key}>
              <p className="text-xs text-gray-400">
                {VISITOR_FIELD_LABELS[key] ?? key}
              </p>
              <p className="text-gray-700 break-words">{value}</p>
            </div>
          ))
        ) : (
          <p className="text-gray-400">Chưa có thông tin context.</p>
        )}
      </div>
    </aside>
  );
